...
# Input 1: A list of original sentence token strings.
# Input 2: A list of corrected sentence token strings.
# Input 3: An original sentence Spacy Object
# Input 4: A corrected sentence Spacy Object
# Input 5: A preloaded Spacy processing object.
# Input 6: Boolean whether to use Levenshtein or not. Default uses DL.
...
def getAutoAlignedEdits(orig_str, cor_str, orig, cor, spacy, lev, merge_strategy):
	global NLP
	NLP = spacy
	...
	# Otherwise, use Linguistically enhanced Damerau-Levenshtein
		...
		alignments = DL.WagnerFischer(orig_str, cor_str, orig, cor, substitution=token_substitution)
	...
	if merge_strategy == "rules": edits = get_edits(orig, cor, get_opcodes(alignment))
	elif merge_strategy == "all-split": edits = get_edits_split(orig, cor, get_opcodes(alignment))
	elif merge_strategy == "all-merge": edits = get_edits_group_all(orig, cor, get_opcodes(alignment))
	elif merge_strategy == "all-equal": edits = get_edits_group_type(orig, cor, get_opcodes(alignment))
	proc_edits = []
	for edit in edits:
		orig_start = edit[1]
		orig_end = edit[2]
		...
		proc_edits.append([orig_start, orig_end, cat, cor_tok, cor_start, cor_end])
	return proc_edits
